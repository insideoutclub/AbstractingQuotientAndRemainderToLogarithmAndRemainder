...
If you use std::plus&ltint&gt as the binary operation, the algorithm will perform multiplication (n x a) in O(log2(n)) steps. If you take the exact same code, but use std::multiplies<int> as the binary operation, the algorithm will perform the operation of raising to a power (a ^ n), again in O(log2(n)) steps. This is one of the fundamental themes of Stepanov's work. Take an algorithm that does something useful in a specific context, figure out the minimal requirements for the types used by that algorithm, and then abstract or expand the algorithm to do something useful in many broader contexts.
